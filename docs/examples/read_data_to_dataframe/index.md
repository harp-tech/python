...
!!! tip
    For a recorded session folder rather than one loose file, use [`open_dataset`](../read_dataset/index.md), which resolves each register against the device schema so any of them can be read by class, by name, or by address.
